<!--
Source: https://github.com/curl/curl/blob/master/docs/tests/FILEFORMAT.md (retrieved 2026-06-10)
Why mimic: a real DECLARATIVE one-file-per-test-case format. The mimic-worthy idea is the
<testcase> skeleton: <info>/<keywords> (metadata + what it covers), <client>/<command>
(setup + invocation), <verify> (expected exit code/output/files). The protocol-specific
tags below are curl-only — study the SHAPE, not the tag list. Verbatim upstream copy below.
-->

...
from athc.scheduler.domain.league import Conference, ConferenceRankings, Division, Team
    ...
    for rank, opp_ranks in FIXED_NONCONF_RANK_OPPONENTS.items():
        if len(opp_ranks) != 3:
            raise SchedulerError(
                f"Rank {rank} must have exactly 3 fixed non-conference opponents"
            )
        if len(set(opp_ranks)) != 3:
            raise SchedulerError(
                f"Rank {rank} has duplicate fixed non-conference opponents"
            )
        for opp_rank in opp_ranks:
            if opp_rank not in FIXED_NONCONF_RANK_OPPONENTS:
                raise SchedulerError(
                    f"Rank {rank} references invalid opponent rank {opp_rank}"
                )
            if rank not in FIXED_NONCONF_RANK_OPPONENTS[opp_rank]:
                raise SchedulerError(
                    f"Fixed non-conference rank table is not symmetric: {rank} -> "
                    f"{opp_rank} without the reverse edge"
                )
...
class FixedMatchupBuilder:
    ...
    def _add_history_matchups(self) -> None:
        afc_remaining = [
            ...
            and self.remaining_nonconference[team] > 0
        ]
        nfc_remaining = [
            team
            for team in self.teams
            if team.conference == Conference.NFC
            and self.remaining_nonconference[team] > 0
        ]
        if {
            self.remaining_nonconference[team] for team in afc_remaining + nfc_remaining
        } - {1}:
            unresolved = {
                team.metro: remaining
                for team, remaining in self.remaining_nonconference.items()
                if remaining > 0
            }
            raise SchedulerError(
                f"History step expected only single-slot teams, got {unresolved}"
            )
    ...
    def _pseudo_inverse_rank_cost(
        ...
        if team_rank < 5:
            direction_scale = (
                UNFAVORABLE_MATCHUP_MULTIPLIER if opp_rank < target_rank else 1
            )
        else:
            direction_scale = (
                UNFAVORABLE_MATCHUP_MULTIPLIER if opp_rank > target_rank else 1
    ...
    def _solve_exact_assignment(
        ...
        if len(left_teams) != len(right_teams):
            raise SchedulerError(
                f"Unbalanced assignment sides: left={len(left_teams)}, "
                f"right={len(right_teams)}"
            )
        ...
        for left in left_teams:
            for right in right_teams:
                ...
                assignment.add_arc_with_cost(
                    left_index[left], right_index[right], cost_fn(left, right)
                )
    ...
    def _solve_four_team_extra_rank_pairs(
        ...
        afc_east = [
            team
            for team in self.ranked_teams_by_conf[Conference.AFC]
            if team.division == Division.AFC_EAST
        ]
        nfc_east = [
            team
            for team in self.ranked_teams_by_conf[Conference.NFC]
            if team.division == Division.NFC_EAST
        ]
        if len(afc_east) != 4 or len(nfc_east) != 4:
            raise SchedulerError(
                "Expected exactly 4 teams per East division for extra SOS assignment"
            )
    ...
    def build_matchup_plan(self) -> MatchupPlan:
        ...
        if len(self.matchups) != 144:
            raise SchedulerError(
                f"Expected 144 total matchups in phase-1 inventory, got "
                f"{len(self.matchups)}"
            )

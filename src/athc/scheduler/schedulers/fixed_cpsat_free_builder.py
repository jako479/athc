"""Phase-1 matchup builder for Scheduler D (fixed-place + CP-SAT, free-only).

Like Scheduler C: two non-conference games per team fixed by division
standings (5th places play each other), one CP-SAT solve picks the rest. The
difference: the difficulty line targets only the picked (non-fixed) games,
tilted by `d_spread`.

Self-contained on purpose: owns its table and difficulty line so the other
schedulers can change independently.
...
# Difficulty line: target average opponent conference rank (1-9) of the
# picked games only. d_spread tilts it (0 = flat at 5).
CONF_RANK_CENTER = 5
CONF_RANK_HALF_RANGE = 4

# Deviations are scored in 1/DIFFICULTY_SCALE-rank units. 6 = LCM(2, 3) keeps
# rank_sum * (DIFFICULTY_SCALE / games) an exact integer for both the 2- and
# 3-picked-game teams.
DIFFICULTY_SCALE = 6
...
def difficulty_target(
    conf_rank: int, d_spread: float = DEFAULT_DIFFICULTY_D_SPREAD
) -> float:
    """Target average picked-opponent conference rank for a team of `conf_rank`."""
    return CONF_RANK_CENTER + d_spread * (conf_rank - CONF_RANK_CENTER) / (
        CONF_RANK_HALF_RANGE
    )


class _FixedCpsatFreeNonConferenceModel:
    """Select every AFC/NFC matchup with the fixed place-table pairs forced in.

    `fixed_pairs` are pinned to 1 before solving; the line objective scores
    only the remaining (picked) games.
    """
    ...
    def __init__(
        ...
        self.fixed_opponents: dict[Team, set[Team]] = {t: set() for t in self.teams}
        for team_a, team_b in fixed_pairs:
            self.fixed_opponents[team_a].add(team_b)
            self.fixed_opponents[team_b].add(team_a)
        # Name "x" is OR-Tools convention; key is [AFC team, NFC team].
        self.x: dict[tuple[Team, Team], cp_model.IntVar] = {}
        self.free_rank_sum: dict[Team, cp_model.IntVar] = {}
    ...
    def _free_games_for(self, team: Team) -> int:
        return nonconference_games_for(team.division) - len(self.fixed_opponents[team])
    ...
    def _add_free_rank_sum_constraints(self) -> None:
        # Sum opponent ranks over the picked games only (fixed pairs excluded).
        for team in self.teams:
            free_games = self._free_games_for(team)
            score = self.model.new_int_var(
                free_games,
                TEAMS_PER_CONFERENCE * free_games,
                f"nc_free_rank_sum_{team.metro}",
            )
            self.model.add(
                score
                == sum(
                    self.conf_rank[opponent] * self._var_for_pair(team, opponent)
                    for opponent in self._opponents_for(team)
                    if opponent not in self.fixed_opponents[team]
                )
            )
            self.free_rank_sum[team] = score
    ...
    def _set_line_objective(self) -> None:
        ...
        for team in self.teams:
            free_games = self._free_games_for(team)
            scaled_sum = self.free_rank_sum[team] * (DIFFICULTY_SCALE // free_games)
            target = round(
                difficulty_target(self.conf_rank[team], self.d_spread)
                * DIFFICULTY_SCALE
            )
    ...
    def build(self) -> None:
        ...
        self._add_free_rank_sum_constraints()
    ...
    def solve(self, seed: int = 0, time_limit: float | None = None) -> set[Matchup]:
        ...
        if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            raise SchedulerError(
                f"Fixed-place + CP-SAT free non-conference model returned status "
...
class FixedCpsatFreeMatchupBuilder:
    def __init__(
        self,
        teams: Sequence[Team],
    ...
    def build_matchup_plan(self) -> MatchupPlan:
        if self.division_standings is None:
            raise SchedulerError(
                "Scheduler D needs the league file's [DivisionStandings] section"
            )

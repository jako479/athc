...
from athc.scheduler.schedulers.types import Matchup, MatchupPlan, Matchups, make_matchup
...
# Deviations are scored in 1/DIFFICULTY_SCALE-rank units against the exact curve
# target. A multiple of both non-conference game counts (4 and 5) keeps
# opponent_rank_sum * (DIFFICULTY_SCALE / games) an exact integer and puts 4- and
# 5-game teams in one unit. 20 = LCM(4, 5).
DIFFICULTY_SCALE = 20
...
class _RankBasedNonConferenceModel:
    """Select every AFC/NFC matchup from rank order: difficulty by overall 1-18
    rank, the one-strong/one-weak floor by derived conference 1-9 rank."""
    ...
    def _add_opponent_rank_sum_constraints(self) -> None:
        for team in self.teams:
            ...
            score = self.model.new_int_var(
                ...
                TOTAL_TEAMS * nonconference_games_for(team.division),
                f"nc_rank_sum_{team.metro}",
            )
            self.model.add(
                score
                == sum(
                    self.overall_rank[opponent] * self._var_for_pair(team, opponent)
                    for opponent in opponents
                )
            )
            self.opponent_rank_sum[team] = score

    def _set_curve_objective(self) -> None:
        # Score each team's deviation from its exact curve target in
        # 1/DIFFICULTY_SCALE-rank units, then minimize the largest deviation
        # (minimax) and, as a tie-break, the total (minisum). The tie-break weight
        # exceeds any possible total, so the largest deviation is minimized first.
        deviations: list[cp_model.IntVar] = []
        max_dev = DIFFICULTY_SCALE * TOTAL_TEAMS
        for team in self.teams:
    ...
    def solve(self, time_limit: float | None = None) -> set[Matchup]:
        ...
        if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            raise SchedulerError(
                f"Rank-only non-conference model returned status "
...
class MatchupBuilder:
    def __init__(
        ...
        self.overall_rank = {team: rankings.overall_rank(team) for team in self.teams}

...
from athc.scheduler.domain.league import Team, ordered_teams
...
def _week_games(schedule: Schedule) -> dict[int, list[Game]]:
    ...
    for game in sorted(
        schedule.games, key=lambda g: (g.week, g.away.metro, g.home.metro)
    ):
...
def _team_games(schedule: Schedule) -> dict[Team, list[Game]]:
    ...
    return {
        team: sorted(schedule.games_for(team), key=lambda g: g.week) for team in teams
    }
...
def _render_nav_links(
    labels: list[tuple[str, str]], columns: int, width: int
) -> list[str]:
    rows: list[str] = []
...
def _named_header(name: str, label: str, width: int) -> str:
    return f'<A NAME="{escape(name)}"></A><b>{escape(label)}</b>' + (
        " " * max(width - len(label), 0)
    )
...
class HtmlScheduleWriter:
    ...
    def render(self, schedule: Schedule) -> str:
        ...
        title = (
            ...
            else f"{self.league_name}, {self.season_label}"
        )
    ...
    def _render_week_by_week(self, schedule: Schedule) -> list[str]:
        ...
        lines.extend(
            [
                ...
                *_render_nav_links(
                    [(f"W{week}", f"Week {week}") for week in weeks],
                    columns=4,
                    width=26,
                ),
                "",
    ...
    def _render_team_by_team(self, schedule: Schedule) -> list[str]:
        ...
        for index, team_pair in enumerate(team_columns):
            ...
            right_header = (
                _named_header(f"T{right_anchor}", right_team.metro, col_width)
            ...
            left_games = [
                _format_team_game(left_team, game) for game in games_by_team[left_team]
            ]
            right_games = (
                [
                    _format_team_game(right_team, game)
                    for game in games_by_team[right_team]
                ]
                if right_team is not None
                else []
            )

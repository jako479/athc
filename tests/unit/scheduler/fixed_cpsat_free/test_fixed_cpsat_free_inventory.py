"""Phase-1 inventory tests for the FixedCpsatFreeMatchupBuilder (Scheduler D).

Like Scheduler C's suite, but the difficulty line covers only the picked
(non-fixed) games. These tests build the plan directly (no week placement),
...
def fixed_cpsat_free_matchup_plan(league: League) -> MatchupPlan:
    return FixedCpsatFreeMatchupBuilder(
        teams=league.teams,
...
def _free_opponents(team: Team, plan: MatchupPlan) -> set[Team]:
    fixed = {
        j if i == team else i
        for i, j in plan.fixed_nonconference_pairs
        if team in (i, j)
    }
    return _nonconference_opponents(team, plan.matchups) - fixed


def _avg_free_opponent_conf_rank(team: Team, plan: MatchupPlan, rankings) -> float:
    free = _free_opponents(team, plan)
    return sum(rankings.rank_of(opp) for opp in free) / len(free)


def test_inventory_has_expected_total_counts(fixed_cpsat_free_matchup_plan, league):
    matchups = fixed_cpsat_free_matchup_plan.matchups
...
def test_inventory_has_expected_divisional_and_conference_counts(
    fixed_cpsat_free_matchup_plan, league
):
    pair_counts = Counter(fixed_cpsat_free_matchup_plan.matchups)
...
def test_inventory_assigns_expected_nonconference_degree(
    ...
    for team in league.teams:
        ...
        actual = len(
            _nonconference_opponents(team, fixed_cpsat_free_matchup_plan.matchups)
        )
        assert actual == expected, f"{team.metro}: wrong non-conference degree"


def test_inventory_contains_fixed_place_table_pairs(
    fixed_cpsat_free_matchup_plan, league
):
    expected = _place_pairs_from(league)
    assert expected <= set(fixed_cpsat_free_matchup_plan.matchups)
    assert fixed_cpsat_free_matchup_plan.fixed_nonconference_pairs == expected


def test_inventory_records_fixed_pairs_per_team(fixed_cpsat_free_matchup_plan, league):
    # 16 teams x 2 fixed + 2 fifth-place teams x 1, halved = 17 pairs.
    fixed = fixed_cpsat_free_matchup_plan.fixed_nonconference_pairs
    assert len(fixed) == 17
    assert fixed <= set(fixed_cpsat_free_matchup_plan.matchups)
    standings = league.division_standings
...
def test_inventory_uses_canonical_pair_ordering(fixed_cpsat_free_matchup_plan):
    assert all(i.metro < j.metro for i, j in fixed_cpsat_free_matchup_plan.matchups)
...
def test_gives_each_team_a_top_and_bottom_half_opponent(
    ...
    for team in league.teams:
        opp_ranks = [
            ...
            for i, j in fixed_cpsat_free_matchup_plan.matchups
...
# ---------------------------------------------------------------------------
# Difficulty line (picked games only)
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("d_spread", [0.0, 1.5, 2.5])
def test_free_difficulty_is_near_line_target(league, d_spread) -> None:
    # Soft target on the picked games only. The picked-opponent pool is fixed
    # by game counts, so steeper tilts saturate: worst observed miss is ~0.9 x
    # spread (0.67 at 0.0, 1.38 at 1.5, 2.25 at 2.5).
    plan = FixedCpsatFreeMatchupBuilder(
        teams=league.teams,
        rankings=league.rankings,
        division_standings=league.division_standings,
        d_spread=d_spread,
    ).build_matchup_plan()
    allowed = max(0.75, d_spread)
    for team in league.teams:
        avg = _avg_free_opponent_conf_rank(team, plan, league.rankings)
        target = difficulty_target(league.rankings.rank_of(team), d_spread)
        assert abs(avg - target) <= allowed, (
            f"{team.metro}: avg picked opponent rank {avg:.2f} far from "
            f"target {target:.2f}"
        )


def test_orders_free_difficulty_by_conference_rank(
    fixed_cpsat_free_matchup_plan, league
) -> None:
    for ranked in (league.rankings.afc, league.rankings.nfc):
        top = _avg_free_opponent_conf_rank(
            ranked[0], fixed_cpsat_free_matchup_plan, league.rankings
        )
        bottom = _avg_free_opponent_conf_rank(
            ranked[-1], fixed_cpsat_free_matchup_plan, league.rankings
        )
        assert top < bottom, "top seed should get tougher picks than the bottom seed"
...
def test_place_table_validation_rejects_invalid_table(monkeypatch, bad_table) -> None:
    monkeypatch.setattr(
        "athc.scheduler.schedulers.fixed_cpsat_free_builder."
        "FIXED_NONCONF_PLACE_OPPONENTS",
        bad_table,
    )
...
def test_builder_errors_without_division_standings(league) -> None:
    builder = FixedCpsatFreeMatchupBuilder(
        teams=league.teams,
        rankings=league.rankings,

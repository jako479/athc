...
These rules are also re-validated end-to-end against each scheduler by `test_schedule_structure.py` / `test_schedule_rules.py` in the `fixed_cpsat/` (Scheduler C) and `fixed_cpsat_free/` (Scheduler D) folders.

...
def test_each_team_plays_exactly_one_game_each_week(schedule, teams):
    for team in teams:
        ...
        assert len(schedule.games_for(team)) == NUM_WEEKS, (
            f"{team.metro}: expected {NUM_WEEKS} total games"
        )
...
def test_each_team_hosts_exactly_eight_games(schedule, teams):
    for team in teams:
        assert len(schedule.home_games_for(team)) == 8, (
            f"{team.metro}: expected exactly 8 home games"
        )
...
def test_no_pair_of_teams_plays_in_back_to_back_weeks(schedule):
    ...
    for pair, weeks in pair_weeks.items():
        weeks.sort()
        for first, second in pairwise(weeks):
            assert second - first > 1, (
                f"{(pair[0].metro, pair[1].metro)}: played in back-to-back weeks {first} and {second}"
            )
    ...
    for i, team_a in enumerate(teams):
        for team_b in teams[i + 1 :]:
            ...
            meetings = [
                game
                for game in schedule.games
                if {game.home, game.away} == {team_a, team_b}
            ]
...
def test_same_conference_cross_division_pairs_appear_once(schedule, teams):
    ...
    for i, team_a in enumerate(teams):
        for team_b in teams[i + 1 :]:
            if (
                team_a.conference == team_b.conference
...
def test_each_team_has_divisional_game_in_last_two_weeks(schedule, teams):
    for team in teams:
        late_games = [
            g for g in schedule.games_for(team) if g.week in (NUM_WEEKS - 1, NUM_WEEKS)
        ]
...
def test_no_three_consecutive_divisional_games_to_start_or_end(schedule, teams):
    for team in teams:
        ...
        end_divisional = sum(
            1
            for g in schedule.games_for(team)
            if g.week in (NUM_WEEKS - 2, NUM_WEEKS - 1, NUM_WEEKS)
            and (g.away if g.home == team else g.home).division == team.division
        )
        assert start_divisional <= 2, (
            f"{team.metro}: opens with 3 straight divisional games"
        )
        assert end_divisional <= 2, (
            f"{team.metro}: ends with 3 straight divisional games"
        )
...
def test_max_one_total_three_game_divisional_streak(schedule, teams):
    for team in teams:
        ...
        assert total_three_game_streaks <= 1, (
            f"{team.metro}: {total_three_game_streaks} divisional streaks of 3+"
        )
...
def test_no_four_consecutive_divisional_games(schedule, teams):
    for team in teams:
        ...
        assert _count_streaks_of(divisional_pattern, 4) == 0, (
            f"{team.metro}: has a divisional streak of 4+ games"
        )
...
def test_at_most_two_divisional_opponents_are_non_interleaved(schedule, teams):
    for team in teams:
        ...
        non_interleaved_opponents = (
            len(divisional_meeting_weeks) - interleaved_opponents
        )
...
def test_max_one_total_home_or_away_three_game_streak(schedule, teams):
    for team in teams:
        ...
        total_three_game_streaks = _count_streaks_of(
            home_pattern, 3
        ) + _count_streaks_of(away_pattern, 3)
        assert total_three_game_streaks <= 1, (
            f"{team.metro}: {total_three_game_streaks} total home/away streaks of 3+"
        )
    ...
    for team in teams:
        ...
        for start in range(NUM_WEEKS - 5):
            ...
            assert 2 <= home_count <= 4, (
                f"{team.metro} weeks {start + 1}-{start + 6}: {home_count} home games in 6-game span"
            )
...
def test_no_three_game_home_or_away_streak_to_start_or_end(schedule, teams):
    for team in teams:
        ...
        assert 1 <= start_home <= 2, (
            f"{team.metro}: invalid home/away split in first 3 weeks"
        )
        assert 1 <= end_home <= 2, (
            f"{team.metro}: invalid home/away split in last 3 weeks"
        )
...
def test_five_team_divisions_have_two_nonconference_home_games(schedule, teams):
    ...
    for team in teams:
        ...
        assert home == 2, (
            f"{team.metro}: expected 2 home non-conference games, got {home}"
        )
        assert away == 2, (
            f"{team.metro}: expected 2 away non-conference games, got {away}"
        )
...
def test_no_four_consecutive_home_or_away_games(schedule, teams):
    for team in teams:
        home_pattern = _home_pattern(schedule, team)
        away_pattern = [not is_home for is_home in home_pattern]
        assert _count_streaks_of(home_pattern, 4) == 0, (
            f"{team.metro}: has a home streak of 4+ games"
        )
        assert _count_streaks_of(away_pattern, 4) == 0, (
            f"{team.metro}: has an away streak of 4+ games"
        )
...
def test_week_16_has_exactly_eight_divisional_games(schedule):
    final_week_divisional_games = sum(
        1
        for game in schedule.games
        if game.week == NUM_WEEKS and game.home.division == game.away.division
    )
